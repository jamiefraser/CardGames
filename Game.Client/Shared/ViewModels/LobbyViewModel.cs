    ...
    public class LobbyViewModel : ViewModelBase,  ILobbyViewModel, IDisposable
    {
        ...
        public async void Initialize(Guid tableId)
        {
            try
            {
                ...
                if (table.InvitedPlayers.Union(table.Players.Values).Where(p => p.PrincipalId.Equals(currentUserService.CurrentClaimsPrincipalOid)).Count() > 0 || table.TableOwner.PrincipalId.Equals(currentUserService.CurrentClaimsPrincipalOid))
                {
                    WaitingForPermissionToJoin = false;
                    nav.NavigateTo($"/games/play/{Table.Id}");
                }
